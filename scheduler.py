...
class Scheduler(object):
    ...
    # 参数：save_mode: 爬取下来的ip使用什么方式保存到文件中,默认按照setting全局配置中的save_mode来保存文件
    ...
    def __init__(self, ip_from="pool", base_url=crawler_base_url, crawler_headers=crawler_headers, crawler_parse_fn=None,crawler_pages=crawler_pages, save_mode=save_mode,check_fn=None, check_max_workers=check_max_worker, test_url=test_url, test_headers=test_headers):
       self.crawler = CrawlerIp(base_url=base_url,headers=crawler_headers,parse_fn=crawler_parse_fn,pages=crawler_pages,save_mode=save_mode)
...
if __name__ == "__main__":
    scheduler = Scheduler(check_max_workers=150,ip_from="web",crawler_pages=10,save_mode="w")
